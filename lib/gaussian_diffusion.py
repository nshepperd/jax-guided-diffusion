...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        # Convert all these to jax arrays
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        return (
            self.sqrt_alphas_cumprod[t].rearrange('b -> b 1 1 1') * x_start
            + self.sqrt_one_minus_alphas_cumprod[t].rearrange('b -> b 1 1 1') * noise
        )
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        new_mean = (
            p_mean_var["mean"] + p_mean_var["variance"] * gradient
        )
        return new_mean
    ...
    def p_sample(self, model, x, t, rng,
                 clip_denoised=True,
                 denoised_fn=None,
                 cond_fn=None,
                 model_kwargs=None):
        ...
        noise = jax.random.normal(rng.split(), x.shape)
        nonzero_mask = (
            (t != 0).reshape([-1] + [1] * (len(x.shape) - 1))
        )  # no noise when t == 0
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = jax.random.normal(rng.split(), shape)

        if skip_timesteps and init_image is None:
            init_image = jnp.zeros_like(img)
